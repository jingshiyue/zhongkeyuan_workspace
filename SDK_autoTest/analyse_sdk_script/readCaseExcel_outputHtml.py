...
from SDK_autoTest.utils.excel_operate import *
from SDK_autoTest.analyse_sdk_script.analyse_sdk_result import *

log_path = "auto_%s.txt" % time.strftime('%Y%m%d%H%M%S',time.localtime())
...
# con_list0 = rd_xls.get_col_data(0)
# con_list1 = rd_xls.get_col_data(1)
# con_list2 = rd_xls.get_col_data(2)   #get_cell_value(self,row,col)
# print(con_list2)
...
def get_html_for_Case(sanJiName,rd_xls,rd_result_xls):
        ...
        if siJiName in suites_name:
            ...
            for jieGuoIdx in jieGuoIdx_list:
                if rd_result_xls.get_cell_value(jieGuoIdx[0],jieGuoIdx[1]) in fuBiaoTi and rd_result_xls.get_cell_value(jieGuoIdx[0],jieGuoIdx[1]-1) in leiMing:
                    ...
                    print(jieguo_list)
                    print(type(jieguo_list))
                    for t in jieguo_list:
                        print(t)
                        print(type(t))
    ...
    if yiJiName:
        ...
        conHtml = conHtml + '<td  class="jieguo" >%s</td>' %jieguo_str +"\n"
        conHtml = conHtml + "</tr>" +"\n"
        return conHtml
    if erJiName:
        conHtml = conHtml + "<tr>" +"\n"
        ...
        conHtml = conHtml + '<td  class="yongli_or_mudi">%s</td>' %mudi +"\n"
        conHtml = conHtml + '<td  class="buzhou">%s</td>' %buzhou
        conHtml = conHtml + '<td colspan="2" class="jieguo" >%s</td>' % jieguo_str + "\n"
        conHtml = conHtml + "</tr>" +"\n"
        return conHtml

def write_html(str):
    f = open(r"D:\workfile\zhongkeyuan_workspace\SDK_autoTest\analyse_sdk_script/result.html","w",encoding="utf-8")
    f.write(str)
    f.close()

if __name__ == '__main__':
    rd_result_xls = rd_excel("result",
                             r"D:/workfile/zhongkeyuan_workspace/SDK_autoTest/analyse_sdk_script/result/result_WIN764_20200102164902.xls")
    rd_xls = rd_excel("Sheet1", r"D:\workfile\zhongkeyuan_workspace\SDK_autoTest\template\excel_case_manage.xlsx") #模板名称
    html = r"D:\workfile\zhongkeyuan_workspace\SDK_autoTest\template/templet_auto.html"

    hf = open(html, "r", encoding='UTF-8')
    contentHtml = ""
    content = hf.readline()
                ...
                for ii in caseManger_dic[yiji]:  # liebiao bianli
                    for erji, vv in ii.items():  # zidian bianli
                        for sanji in vv:  #
                            ...
                            # logging.debug(tmp)
    hf.close()
    write_html(contentHtml)
    logging.debug("生成html成功...")
