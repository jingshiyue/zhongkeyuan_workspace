...
if __name__ == "__main__":
    ...
    print("monitorF",monitorF)
    mf = open(monitorF, "r")
    cpu,ram,t = [],[],[]
    start_cont = mf.readline()
    start_time = start_cont[0:19]
    _start = str2timestamp(start_time)
    _xticks = []
    while (start_cont):
        if "Memory" in start_cont:
            _memory = float((find_str(start_cont, "Memory", 3)))
        if "CPU" in start_cont:
            _cpu = float((find_str(start_cont, "CPU", 5)))
        if "2020" in start_cont:
            _t = '%.2f' % ((str2timestamp(start_cont[0:19]) - _start) / 3600)
            _t = float(_t)
        cpu.append(_cpu)
        ram.append(_memory)
        t.append(_t)
