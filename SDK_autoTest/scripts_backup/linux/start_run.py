...
if __name__=="__main__":   
    ...
    try:
        ...
        s=subprocess.Popen(r"python3 ./test.py",shell=True,bufsize=0,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True)
        while True:
            nextline = "decode failed"
            try:
                nextline=s.stdout.readline()
            except:
                logging.debug(nextline)
            logging.debug(nextline.strip())
            if "cases in test.py have complated" in nextline:
                break
