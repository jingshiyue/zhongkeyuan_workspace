...
"""1.,复核闸机1 在底库3000人时，对同一个进行并发30压力测试，
并且传入的特征为最后一次注册后人员的特征，服务器响应时间小于1s，持续时间1小时
测试步骤，
1.先建立底库为3000人，图片索引0-2999
2.随机对3000人底库进行请求发送
3.查看服务器响应时间，以及识别率情况
"""


class ReviewAutoZHAji1(TaskSequence):
    """2.3.16 自助闸机复核接口（二期）"""

    @staticmethod
    def build_lib():
        """通过人工通道验票接口建立3000人的底库,直接造成航班号为图片名称
        """
        # 先生成3000个不同的身份证号码
        list_id = produce_flight_list_only(3000)
    ...
    @task
    def post_review_anto_zhaji_1(self):
...
class BehaviorReview1(HttpLocust):
    task_set = ReviewAutoZHAji1
    max_wait = 1000
    min_wait = 1000
...
if __name__ == '__main__':
    ...
    command3 = r"locust -f review_zhaj_1.py -P 8095 --web-host=127.0.0.1"
    os.system(command1)
    os.system(command2)
    os.system(command3)