...
import json
"""
（2）、底库为5000，对同一个进行并发30压力测试，并且传入的特征为最后一次注册后人员的特征，服务器响应时间小于1s，同时，返回状态正确
"""

"""操作步骤
1.通过刷票接口进行注册5000人作为底库
2.对注册的最后一人进行压测，
...
class TaskReview1N1(TaskSequence):
    ...
    def first_task(self):
        ...
        body = {"reqId": get_uuid(),
                ...
                "sceneFeature": get_txt(shiwanid2k_features+"/"+str(4999)+".txt")}
        with self.client.post(url="/review-server/api/v1/face/review/check",
            ...
            try:
                ...
                if json_data["result"] == 0 and json_data["userInfo"]["flightNumber"] == "flightno4999":
                    response.success()
                elif json_data["result"] == 0 and json_data["userInfo"]["flightNumber"] != "flightno4999":
...
if __name__ == '__main__':
    ...
    command3 = r"locust -f review_1N2.py -P 8103 --web-host=127.0.0.1"
    os.system(command1)
    os.system(command2)
    os.system(command3)