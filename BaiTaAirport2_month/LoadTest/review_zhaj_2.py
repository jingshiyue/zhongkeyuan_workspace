...
"""2.在底库5000人时，对同一个进行并发30压力测试，
并且传入的特征为最后一次注册后人员的特征，服务器响应时间小于？，持续时间1小时
测试步骤，
1.先建立底库为5000人，图片索引0-4999
2.对4999人索引底库进行请求发送
3.查看服务器响应时间，以及识别率情况
"""


class ReviewAutoZHAji2(TaskSequence):
    """2.3.16 自助闸机复核接口（二期）"""
    ...
    def build_lib():
        ...
        """
        # 先生成3000个不同的身份证号码
        list_id = get_random_idcard_to_only_list(5000)
    ...
    @task
    def post_review_anto_zhaji_2(self):
...
if __name__ == '__main__':
    ...
    command3 = r"locust -f review_zhaj_2.py -P 8096 --web-host=127.0.0.1"
    os.system(command1)
    os.system(command2)
    os.system(command3)