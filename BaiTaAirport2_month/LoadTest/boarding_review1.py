...
import json
"""1、一个航班（1000）人时，
随机本次航班1000人特征进行30并发压力，
时间小于1s，识别成功率100%"""

"""操作步骤
1.先通过刷票接口建立一个航班1000人的底库
2.在对本次航班人员（随机取值）进行30并发压力测试
3.查看服务器响应情况
"""
...
class BoardingReview1(TaskSequence):
    ...
    def post_boarding_review1(self):
        ...
        body = {"reqId": get_uuid(),
                "faceImage": to_base64(r"C:\chenkeyun\OtherFile\IDcard\0.jpg"),
                "faceFeature": get_txt(shiwanid2k_features+"/"+str(random_index)+".txt"),
                "deviceCode": "T3DJ001",
                "boardingGate": "15",
                "flightNo": "flight01",
        ...
        with self.client.post(url="/boardinggate-server/api/v1/face/boarding/check",
            ...
            try:
                if json_data["status"] == 0 and json_data["userInfo"]["boardingNumber"] == board:
                    response.success()
                    print(json_data)
                else:
...
if __name__ == '__main__':
    ...
    command3 = r"locust -f boarding_review1.py -P 8098 --web-host=127.0.0.1"
    os.system(command1)
    os.system(command2)
    os.system(command3)